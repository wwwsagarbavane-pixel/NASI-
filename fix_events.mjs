...
// The new events to append (if they exist, I will replace them entirely, or just replace the end of the file)
const newEvents = `
  {
    "id": "mock_event_1",
  ...
  },
  {
    ...
    "parentSection": "events",
    "content": "A symposium on Emerging Trends in Seed Industry was organized by NSAI and Society for Plant Research. The themes include conventional and modern breeding approaches, genome editing, and modern phenotyping.",
    "images": [
  ...
  },
  {
    ...
    "parentSection": "events",
    "content": "Agriculture Today Group held a roundtable meeting on pre-kharif seeds at Constitution Club, New Delhi to address key industry trends.",
    "images": [
...
// First remove any existing mock events to prevent duplication
content = content.replace(/,\s*\{\s*"id": "mock_event_1"[\s\S]*\}\s*\];/m, '];');

// Replace the closing bracket of the array with our new events
content = content.replace(/\];[\s\n]*$/, ',' + newEvents);

fs.writeFileSync(EVENTS_FILE, content, 'utf8');
console.log('Fixed mock events with image arrays!');
